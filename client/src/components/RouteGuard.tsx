...
export default function RouteGuard({ children, allowedRoles }: Props) {
  ...
  useEffect(() => {
    ...
    // Widget is for doctors, cashiers, IT admins, lab technicians, and pharmacists
    console.log('[RouteGuard] Widget loading check - User role:', user?.role);
    const shouldLoadWidget = user?.role === 'Doctor' || user?.role === 'Cashier' || user?.role === 'ITAdmin' || user?.role === 'LabTech' || user?.role === 'Pharmacist';
    ...
    const loadWidget = async () => {
      try {
        ...
        // Fetch from admin integration for doctors, cashiers, IT admins, lab technicians, and pharmacists, patient portal integration for others
        let embed = null;
        if (user.role === 'Doctor' || user.role === 'Cashier' || user.role === 'ITAdmin' || user.role === 'LabTech' || user.role === 'Pharmacist') {
        ...
        else if(user?.role === 'Cashier' || user?.role === 'ITAdmin' || user?.role === 'LabTech' || user?.role === 'Pharmacist') {
  ...
  // Widget is for doctors, cashiers, IT admins, lab technicians, and pharmacists
  const shouldShowWidget = !!widgetFrame && (user?.role === 'Doctor' || user?.role === 'Cashier' || user?.role === 'ITAdmin' || user?.role === 'LabTech' || user?.role === 'Pharmacist');
